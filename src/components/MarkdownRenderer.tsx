...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ 
  ...
  const renderContent = () => {
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith('# ')) {
        elements.push(
          <h2 key={i} className="text-2xl font-semibold tracking-tight mb-4">
            {line.slice(2)}
          </h2>
        );
      }
      // Sub headings
        ...
        elements.push(
          <h3 key={i} className="text-lg font-medium text-white mb-3 mt-2">
            {line.slice(3)}
          </h3>
        );
      }
      // Separator
        ...
        elements.push(
          <hr key={i} className="border-gray-600/60 my-4" />
        );
      }
      // Lists
      else if (line.startsWith('- ')) {
        ...
        elements.push(
          <div key={startIndex} className="mac-panel-soft p-4">
      ...
      // Links
        ...
        if (match) {
          elements.push(
            <div key={i} className="mac-panel-soft p-4">
              ...
              <a
                ...
                className="inline-flex items-center space-x-2 mac-button mac-button-primary text-sm transition-colors"
              >
